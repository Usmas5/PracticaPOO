...
def main():
    ...
    while True:
        ...
        if opcion == 1:
            ...
            #Se solicita la cedula o nombre a buscar
            identificador = input("Ingrese la cédula o el nombre a buscar: ")
            #Se buscan los pacientes en los datos
            pacientes = sis.verDatosPaciente(identificador)
            #Verifica si el paciente se encuentra
            if pacientes is not None:
                print("Datos del paciente(s):")
                for p in pacientes:
                    print("Nombre:", p.verNombre())
                    print("Cédula:", p.verCedula())
...
#python descubre cual es la funcion principal
